...
AddEventHandler("route68_kasyno:KupZetony", function(count)
    ...
    if xPlayer then
        ...
        local kwota = math.floor(count * 5)
        if kwota > cash then
            TriggerClientEvent('pNotify:SendNotification', _source, {text = 'Não tens dinheiro suficiente!'})
        elseif kwota <= cash then
            xPlayer.addInventoryItem('zetony', count)
            xPlayer.removeMoney(kwota)
            TriggerClientEvent('pNotify:SendNotification', _source, {text = 'As tuas '..count..' Fichas dão $'..kwota..'.'})
        end
    end
end)
...
AddEventHandler("route68_kasyno:KupAlkohol", function(count, item)
    ...
    if xPlayer then
        ...
        if kwota > cash then
            TriggerClientEvent('pNotify:SendNotification', _source, {text = 'Parece que estás Liso, não tens dinheiro suficiente!'})
        elseif kwota <= cash then
            xPlayer.addInventoryItem(item, count)
            xPlayer.removeMoney(kwota)
            TriggerClientEvent('pNotify:SendNotification', _source, {text = 'Os teus '..count..' items dão $'..count..'.'})
        end
    end
end)
